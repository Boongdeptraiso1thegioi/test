do
  ...
  RandomCFrame = false
  _B = true
  -- [FIXED] Andepzai 100% Spin System - Toc do nhanh
  _G.SpinAngle = 0
  _G.SpinRadius = 6.5
  _G.SpinSpeed = 50
...
-- [FIX] Cho game load co gioi han thoi gian (tranh treo khi inject sau khi da load xong)
if not game:IsLoaded() then game.Loaded:Wait() end
do
    local t0 = tick()
    repeat task.wait(0.1) until (plr.PlayerGui:FindFirstChild("Main") ~= nil) or (tick() - t0 > 15)
    local mainGui = plr.PlayerGui:FindFirstChild("Main")
    local loading = mainGui and mainGui:FindFirstChild("Loading")
    if loading then
        local t1 = tick()
        repeat task.wait(0.2) until (not loading.Parent) or (loading.Visible == false) or (tick() - t1 > 30)
    end
end
...
-- [FIXED] Andepzai 100% Style Orbit - toc do nhanh gap 5x
function GetSpinCFrame(targetPart, offset)
    if not RandomCFrame then
        return targetPart.CFrame * CFrame.new(0, offset, 0) * CFrame.Angles(0, math.rad(180), 0)
    end
    -- Toc do xoay nhanh nhu Andepzai
    _G.SpinAngle = (_G.SpinAngle + (_G.SpinSpeed or 50)) % 360
    local radius = _G.SpinRadius or 6.5
...
-- [FIX] Thuc su tung don danh (truoc day chi bay quanh mob ma khong danh)
local _vu = game:GetService("VirtualUser")
Attack.Click = function()
    pcall(function()
        _vu:CaptureController()
        _vu:Button1Down(Vector2.new(1280, 672))
        _vu:Button1Up(Vector2.new(1280, 672))
    end)
end
-- Tu chon vu khi neu chua chon trong menu
Attack.GetWeapon = function()
    if _G.SelectWeapon and (plr.Backpack:FindFirstChild(_G.SelectWeapon) or (plr.Character and plr.Character:FindFirstChild(_G.SelectWeapon))) then
        return _G.SelectWeapon
    end
    local char = plr.Character
    if char then
        local eq = char:FindFirstChildOfClass("Tool")
        if eq then return eq.Name end
    end
    for _, v in ipairs(plr.Backpack:GetChildren()) do
        if v:IsA("Tool") and (v.ToolTip == "Melee" or v.ToolTip == "Sword" or v.ToolTip == "Blox Fruit" or v.ToolTip == "Gun") then
            return v.Name
        end
    end
    return nil
...
Attack.Kill = function(model, Succes)
    ...
    while model.Parent and model:FindFirstChild("Humanoid") and model.Humanoid.Health > 0 do
        ...
        pcall(function()
            if isFastMob then
                -- Farm Bone & Katakuri bay 3,000,000 cuc nhanh
                _tpFast(GetSpinCFrame(current.HumanoidRootPart, offset))
            else
...
function CanBypassTeleport(x)
    ...
    local minDist = _G.BypassMinDistance or 3500
    if GetDistance(x.Position) <= minDist then
        return false
    end
...
-- [NEW] Farm Bone & Katakuri FAST MODE - Bay đến đảo 300 bth, farm 3000000
_tpFast = function(target)
    local gg
    if typeof(target) == "Vector3" then
        gg = CFrame.new(target)
    elseif typeof(target) == "CFrame" then
    ...
    local distance = (gg.Position - rootPart.Position).Magnitude
    local tweenInfo = TweenInfo.new(distance / 3000000, Enum.EasingStyle.Linear)
    local tween = game:GetService("TweenService"):Create(block, tweenInfo, {CFrame = gg})    
...
local redzlib = loadstring(game:HttpGet("https://raw.githubusercontent.com/tranvanhung040419826-ui/Charcoal-UI/refs/heads/main/8469605765224156.lua.txt"))()

local Window = redzlib:MakeWindow({
    Title = "Vantablack X Hub : Blox Fruit",
    SubTitle = "By Trần Duy Khánh",
    SaveFolder = "VantablackX.json"
})
...
local Tabs = {
    ...
    Hop = Window:MakeTab({ Title = "Hop Server [Beta]", Icon = "locate" }),
...
local ConfigFile = "Vantablack_AutoSave.json"
local VantaConfig = {}

if isfile and isfile(ConfigFile) then
    pcall(function()
        VantaConfig = HttpService:JSONDecode(readfile(ConfigFile))
    end)
end
...
local function SaveConfig()
    if writefile then
        pcall(function()
            writefile(ConfigFile, HttpService:JSONEncode(VantaConfig))
        end)
    end
end

-- Không lưu setting cho Tab Hop
for TabName, Tab in pairs(Tabs) do
    if TabName ~= "Hop" then
        if Tab.AddToggle then
            local OldToggle = Tab.AddToggle
            Tab.AddToggle = function(self, data)
                local name = data.Name or "Unnamed_Toggle"
                
                if VantaConfig[name] ~= nil then
                    data.Default = VantaConfig[name]
                end
                
                local oldCb = data.Callback
                data.Callback = function(val)
                    VantaConfig[name] = val
                    SaveConfig()
                    if oldCb then
                        pcall(oldCb, val)
                    end
                ...
                local toggle = OldToggle(self, data)
                
                if VantaConfig[name] ~= nil and oldCb then
                    task.spawn(function() pcall(oldCb, VantaConfig[name]) end)
                end
        ...
        if Tab.AddDropdown then
            local OldDrop = Tab.AddDropdown
            Tab.AddDropdown = function(self, data)
                local name = data.Name or "Unnamed_Dropdown"
...
local function StartHopping(ApiUrl, globalVar, toggleName)
    task.spawn(function()
        local visited = _G.VisitedServers

        while getgenv()[globalVar] do
            ...
            if ok and res and res.Body then
                ...
                if success and body then
                    local server_list = body.all_servers or body.moon_data or body.servers or body
                    if type(server_list) == "table" then
                        for _, v in pairs(server_list) do
                            local job = tostring(type(v) == "table" and (v.jobid or v.jobId or v.id or "") or v)
                            if #job > 10 and job ~= game.JobId and not visited[job] then
                                table.insert(foundJobs, job)
                            end
                        end
                    end
                end
                ...
                if #foundJobs > 0 then
                    isTeleporting = true
                    local TeleportRemote = ReplicatedStorage:FindFirstChild("__ServerBrowser")
                    
                    for i = 1, #foundJobs do
                        if not getgenv()[globalVar] then break end
                        
                        local selected = foundJobs[i]
                        visited[selected] = true
                        
                        Window:Notify({
                            Title = "Vantablack X Hub",
                        ...
                        task.spawn(function()
                            if TeleportRemote then
                                pcall(function() TeleportRemote:InvokeServer("teleport", selected) end)
                            else
                                pcall(function() TeleportService:TeleportToPlaceInstance(game.PlaceId, selected, LocalPlayer) end)
                            end
                        end)
...
-- Đã thêm tham số 'Description' (Mô tả tiếng việt)
local function CreateHopToggle(Tab, Name, Description, ApiUrl)
    local globalVar = "Hop_" .. Name:gsub("%s+", "")
    getgenv()[globalVar] = false
    
    Tab:AddToggle({
        Name = Name,
        Description = Description, -- Hiển thị chữ nhỏ tiếng Việt ở dưới
        Default = false,
        Callback = function(Value)
            getgenv()[globalVar] = Value
            if Value then StartHopping(ApiUrl, globalVar, Name) end
        end
    })
end

if currentPlaceId == Sea2 or currentPlaceId == Sea3 then
    CreateHopToggle(Tabs.Hop, "Auto Hop Full Moon", "Tự động tìm máy chủ đang có trăng tròn.", "https://apifullmoon.vercel.app") 
    CreateHopToggle(Tabs.Hop, "Auto Hop Near Full Moon", "Tự động tìm máy chủ sắp có trăng tròn.", "https://apinear.vercel.app") 
end

if currentPlaceId == Sea2 then
    CreateHopToggle(Tabs.Hop, "Auto Hop Legendary Sword Dealer", "Tìm máy chủ có người bán kiếm huyền thoại.", "https://api-legendarysworddealer.vercel.app") 
    CreateHopToggle(Tabs.Hop, "Auto Hop Cursed Captain", "Tìm máy chủ có thuyền trưởng bị nguyền rủa.", "https://api-cursed-captain.vercel.app") 
    CreateHopToggle(Tabs.Hop, "Auto Hop Darkbeard", "Tìm máy chủ có trùm râu đen.", "https://apidarkbeard.vercel.app") 

elseif currentPlaceId == Sea3 then
    CreateHopToggle(Tabs.Hop, "Auto Hop Mirage Island", "Tìm máy chủ đang có đảo bí ẩn.", "https://apimirageisland.vercel.app") 
    CreateHopToggle(Tabs.Hop, "Auto Hop Kitsune Island", "Tìm máy chủ đang có đảo hồ ly.", "https://api-kitsune.vercel.app") 
    CreateHopToggle(Tabs.Hop, "Auto Hop Elite Hunter", "Tìm máy chủ có trùm tinh anh.", "https://apielite.vercel.app") 
    CreateHopToggle(Tabs.Hop, "Auto Hop Rip Indra", "Tìm máy chủ có trùm Indra.", "https://apirip-indra.vercel.app") 
    CreateHopToggle(Tabs.Hop, "Auto Hop Dough King", "Tìm máy chủ có vua bột nhào.", "https://apidough-king.vercel.app") 
    CreateHopToggle(Tabs.Hop, "Auto Hop Cake Prince", "Tìm máy chủ có hoàng tử bánh.", "https://api-cakeprince.vercel.app") 
    CreateHopToggle(Tabs.Hop, "Auto Hop Near Cake Prince", "Tìm máy chủ sắp đủ quái để gọi hoàng tử bánh.", "https://api-nearcakeprince.vercel.app") 
    CreateHopToggle(Tabs.Hop, "Auto Hop Cake Queen", "Tìm máy chủ có nữ hoàng bánh.", "https://api-cakequeen.vercel.app") 
    CreateHopToggle(Tabs.Hop, "Auto Hop Soul Reaper", "Tìm máy chủ có tử thần.", "https://api-soul-reaper.vercel.app") 
    CreateHopToggle(Tabs.Hop, "Auto Hop Prehistoric Island", "Tìm máy chủ có đảo tiền sử.", "https://api-prehistoric-island.vercel.app") 
    CreateHopToggle(Tabs.Hop, "Auto Hop Tyrant of the Skies", "Tìm máy chủ có bạo chúa bầu trời.", "https://apityrant-of-the-skies.vercel.app") 
    
elseif currentPlaceId == Sea1 then
    Tabs.Hop:AddParagraph("Server Hop events are not available in Sea 1.", "Các sự kiện Server Hop không có sẵn tại Sea 1.")
end

Tabs.Hop:AddSection("And Another Vantablack Hub Script")
Tabs.Hop:AddButton({
    Name = "Copy Full Farm Fruit Script",
...
Tabs.Hop:AddButton({
    Name = "Copy Full Farm Chest Script",
...
Tabs.Info:AddDiscordInvite({
    Title = "Vantablack Hub | Free",
    Description = "Vantablack Hub | Miễn Phí",
    Banner = "rbxassetid://127729348105692", 
    Logo = "rbxassetid://127729348105692",
    Invite = "https://discord.gg/v2e8ZB6uk",
    Members = 1000000, 
    Online = 1000000, 
})
...
FarmLevel = Tabs.Main:AddToggle({
    ...
    Description = "Tự động cày cấp độ chính",
    Default = false,
    Callback = function(Value)
        _G.Level = Value
        if Value then
            -- [FIX] Tat cac farm khac tranh xung dot
            ...
            _G.FarmEliteH = false
            _G.ActiveFarm = "Level"
            print("[FIX] Farm Level ON - Da tat Bone/Near tranh xung dot")
        else
            alreadyTeleported = false
            teleporting = false
            if _G.ActiveFarm == "Level" then
                _G.ActiveFarm = nil
            end
        end
    end
...
ChestBP = Tabs.Main:AddToggle({
    ...
    Description = "Tự động bỏ qua rương đặc biệt",
    Default = false,
    Callback = function(Value)
...
Cake = Tabs.Main:AddToggle({
    ...
    Description = "Tự động farm Hoàng tử Bánh",
    Default = false,
    Callback = function(Value)
    _G.Auto_Cake_Prince = Value
    if Value and (_G.AcceptQuestC == nil or _G.AcceptQuestC == false) then _G.AcceptQuestC = true end
end
})
    ...
    while task.wait() do
        if _G.Auto_Cake_Prince and not _G.AutoRaidCastle then
            pcall(function()
                ...
                if _G.AcceptQuestC ~= false and questUI and not questUI.Visible then
                    local questPos = CFrame.new(-1927.92, 37.8, -12842.54)
                    _tp(questPos)
                    local t0 = tick()
                    while (questPos.Position - root.Position).Magnitude > 30 do
                        task.wait(0.2)
                        _tp(questPos)
                        if not _G.Auto_Cake_Prince or tick() - t0 > 15 then break end
                    ...
                    for _, q in ipairs(questData) do
                        pcall(function()
                            game.ReplicatedStorage.Remotes.CommF_:InvokeServer(unpack(q))
                        end)
                        task.wait(0.2)
                        if questUI.Visible then break end
                    end
                end
...
Name = "Accept Quests", 
Description = "Tự động nhận nhiệm vụ", 
Default = false,
Callback = function(Value)
...
Tabs.Main:AddToggle({
    ...
    Callback = function(Value)
        ...
        if Value then
            -- [FIX] Tu dong bat nhan nhiem vu Bone (truoc day phai bat tay nen khong nhan quest)
            if _G.AcceptQuestB == nil or _G.AcceptQuestB == false then _G.AcceptQuestB = true end
            -- [FIX] Tat farm khac tranh xung dot chet xong van farm bone
            _G.Level = false
            _G.AutoFarmNear = false
            _G.AutoFarmIsland = false
            _G.ActiveFarm = "Bone"
            print("[FIX] Farm Bone ON - Da tat Level tranh xung dot")
        else
            if _G.ActiveFarm == "Bone" then
                _G.ActiveFarm = nil
            end
        end
    end
...
spawn(function()
    ...
    while wait(0.5) do
        ...
        pcall(function()
            ...
            -- [FIX] Nhan nhiem vu TRUOC khi di danh
            if _G.AcceptQuestB ~= false and not hasQuest then
                ...
                until not _G.AutoFarm_Bone
                   or (questPos.Position - (player.Character and player.Character.HumanoidRootPart.Position or root.Position)).Magnitude <= 20
                   or tick() - t0 > 15
                ...
                for _, q in ipairs(questData) do
                    ...
                    wait(0.2)
                    if questUI and questUI.Visible then break end
                end
                return
            end
...
FarmTyrant = Tabs.Main:AddToggle({
    Name = "Auto Farm Boss Tyrant of the Skies", 
    Description = "Tự động farm Boss Bạo Chúa Bầu Trời", 
    Default = false,
    Callback = function(Value) 
        _G.FarmTyrant = Value 
    end
...
-- Tạo một danh sách (Dictionary) để check quái nhanh và mượt hơn
local mobList = {
    ["Serpent Hunter"] = true,
    ["Skull Slayer"] = true,
    ["Isle Champion"] = true,
...
-- Sử dụng task.spawn thay vì spawn (đã lỗi thời) để hiệu suất tốt hơn
task.spawn(function()
    while task.wait() do -- Dùng task.wait() thay vì wait(Sec)
        if _G.FarmTyrant then
            pcall(function()
                ...
                -- ====================================================
                -- 1. ƯU TIÊN TÌM VÀ ĐÁNH BOSS TRƯỚC
                -- ====================================================
                local boss = workspace.Enemies:FindFirstChild("Tyrant of the Skies")
                
                -- Check xem boss có tồn tại, có máu và phần thân (HumanoidRootPart) không
                if boss and boss:FindFirstChild("Humanoid") and boss.Humanoid.Health > 0 and boss:FindFirstChild("HumanoidRootPart") then
                    ...
                    repeat
                        ...
                        -- Nếu boss ở xa hơn tầm đánh (15 studs), tự động TP đến vị trí THỰC TẾ của boss
                        if (hrp.Position - boss.HumanoidRootPart.Position).Magnitude > 15 then
                            -- Bay trên đầu boss 20 studs (có thể tùy chỉnh số 20) để tránh bị kẹt
                    ...
                    return -- Đánh xong boss thì quay lại vòng lặp while để check tiếp, không đánh quái nhỏ
                end

                -- ====================================================
                -- 2. NẾU KHÔNG CÓ BOSS, TÌM QUÁI NHỎ TRONG MAP ĐỂ FARM
                -- ====================================================
                for _, mob in pairs(workspace.Enemies:GetChildren()) do
                    if not _G.FarmTyrant then break end
                    
                    -- Quét toàn bộ thư mục Enemies, nếu tên quái trùng với mobList và còn sống
                    if mobList[mob.Name] and mob:FindFirstChild("HumanoidRootPart") and mob:FindFirstChild("Humanoid") and mob.Humanoid.Health > 0 then
                        
                        repeat
                            task.wait()
                            if not _G.FarmTyrant then break end
                            
                            -- Cập nhật và bám sát vị trí của quái liên tục
...
-- [UPGRADED] Spin System Andepzai Style
RandomAround = Tabs.Settings:AddToggle({
Name = "Auto Turn on Spin", 
Description = "Tự động bật xoay vòng khi farm", 
Default = false,
Callback = function(Value)
  RandomCFrame = Value
...
Tabs.Settings:AddToggle({
    ...
    Description = "Bỏ qua hiệu ứng dịch chuyển",
    Default = true,
    Callback = function(Value)
...
Description = "Tự động bay tới trái", 
Default = false,
Callback = function(Value)
  _G.TwFruits = Value
end})
...
Description = "Tự động chọn chip hầm ngục", 
Default = false,
Callback = function(Value)
...
Tabs.Raids:AddToggle({
    ...
    Description = "Lấy trái dưới 1M trong túi",
    Default = false,
    Callback = function(Value)
...
Description = "Mua chip hầm ngục bằng Beli",
Callback = function()
...
Description = "Mua chip hầm ngục bằng trái",
Callback = function()
...
AutoChipBeli = Tabs.Raids:AddToggle({
    ...
    Description = "Tự động mua chip bằng Beli",
    Default = false,
    Callback = function(Value)
...
AutoChipFruit = Tabs.Raids:AddToggle({
    ...
    Description = "Tự động mua chip bằng trái",
    Default = false,
    Callback = function(Value)
...
Raiding = Tabs.Raids:AddToggle({
    ...
    Description = "Tự động raid + qua đảo tiếp theo",
    Default = false,
    Callback = function(Value)
        _G.Raiding = Value
    end
...
Tabs.Raids:AddToggle({
Name = "Auto Awakening", 
Description = "Tự động thức tỉnh trái", 
Default = false,
Callback = function(Value)
...
Tabs.Raids:AddToggle({
    ...
    Description = "Tự động bay tới phòng thí nghiệm",
    Default = false,
    Callback = function(Value)
        _G.TpLab = Value
        StopTween(_G.TpLab)
...
Tabs.Raids:AddToggle({
    ...
    Description = "Tự động mua chip Law",
    Default = false,
    Callback = function(Value)
...
Tabs.Raids:AddToggle({
    ...
    Description = "Tự động bắt đầu raid Law",
    Default = false,
    Callback = function(Value)
...
Tabs.Raids:AddToggle({
    ...
    Description = "Tự động farm hầm ngục",
    Default = false,
    Callback = function(Value)
...
Tabs.Raids:AddToggle({
    Name = "TP Exit (1)",
    Description = "Dịch chuyển thoát (1)",
    Default = false,
    Callback = function(v)
        _G.TPFloor1 = v
    end
...
Tabs.Raids:AddToggle({
    Name = "TP Exit (2)",
    Description = "Dịch chuyển thoát (2)",
    Default = false,
    Callback = function(v)
        _G.TPFloor2 = v
    end
...
Tabs.Raids:AddToggle({
    Name = "TP Exit (3)",
    Description = "Dịch chuyển thoát (3)",
    Default = false,
    Callback = function(v)
        _G.TPFloor3 = v
    end
...
Tabs.Raids:AddToggle({
    Name = "TP Exit (4)",
    Description = "Dịch chuyển thoát (4)",
    Default = false,
    Callback = function(v)
        _G.TPFloor4 = v
    end
...
local function SafeNotify(title, msg, dur)
    pcall(function()
        if Window and Window.Notify then
            Window:Notify({Title = title, Content = msg, Duration = dur or 3})
        end
    end)
end

local MeleeDropdown = Tabs.Shop:AddDropdown({
    Name = "Select the Melee you want to buy.",
    ...
    Callback = function(Value)
        if Value ~= "Không có võ ở Sea này" then
            SelectedMelee = Value
            SafeNotify("Vantablack Hub", "Đã chọn: " .. Value, 2)
        end
    end
})
...
Tabs.Shop:AddToggle({
    ...
    Callback = function(Value)
        ...
        if Value then
            SafeNotify("Vantablack Hub", "Đang bay đến: " .. SelectedMelee, 3)
        else
            SafeNotify("Vantablack Hub", "Đã tắt Auto Buy Melee.", 2)
        end
            ...
            while _G.AutoBuyMelee do
                ...
                local ok = pcall(function()
                    ...
                    if not data or not data.Pos then
                        SafeNotify("Vantablack Hub", "Võ này không có ở Sea này!", 3)
                    ...
                    if dist > 10 then
                        ...
                        -- HIỆN THÔNG BÁO VÀ TẮT AUTO BUY ĐỂ CHỐNG SPAM
                        if buySuccess then
                            SafeNotify("Vantablack Hub", "Mua/Trang bị thành công: " .. SelectedMelee, 3)
                        else
                            SafeNotify("Vantablack Hub", "Lỗi khi mua: " .. SelectedMelee, 3)
                        end
...
Tabs.Misc:AddButton({
    Name = "Move to the player with the lowest score.", 
...
Tabs.Misc:AddButton({
    Name = "Switch to the server with the lowest latency.", 
...
--------------------------------------------------------------------------------
-- HÀM DÙNG CHUNG (TỐI ƯU HÓA & CHẠY NGẦM)
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
task.spawn(function()
    local WEB_URL = "https://apifullmoon.vercel.app/update-moon" 
...
--------------------------------------------------------------------------------
task.spawn(function()
    local WEB_URL = "https://apinear.vercel.app/update-near-moon" 
...
--------------------------------------------------------------------------------
task.spawn(function()
    local WEB_URL = "https://apimirageisland.vercel.app/update-mirage-island"
    local previousMirageStatus = ""
...
--------------------------------------------------------------------------------
task.spawn(function()
    local WEB_URL = "https://apielite.vercel.app/update-elite" 
...
--------------------------------------------------------------------------------
task.spawn(function()
    local WEB_URL = "https://apidarkbeard.vercel.app/update-darkbeard" 
...
--------------------------------------------------------------------------------
task.spawn(function()
    local WEB_URL = "https://apidough-king.vercel.app/update-doughking" 
...
--------------------------------------------------------------------------------
task.spawn(function()
    local WEB_URL = "https://apirip-indra.vercel.app/update-ripindra" 
...
--------------------------------------------------------------------------------
task.spawn(function()
    local WEB_URL = "https://api-cursed-captain.vercel.app/update-cursed-captain" 
    local previousStatus = ""
...
--------------------------------------------------------------------------------
task.spawn(function()
    local WEB_URL = "https://api-legendarysworddealer.vercel.app/update-legendary-sword-dealer" 
    local previousStatus = ""
...
--------------------------------------------------------------------------------
task.spawn(function()
    local WEB_URL = "https://api-cakequeen.vercel.app/update-cake-queen" 
    local previousStatus = ""
...
--------------------------------------------------------------------------------
task.spawn(function()
    local WEB_URL = "https://api-cakeprince.vercel.app/update-cake-prince" 
    local previousStatus = ""
...
--------------------------------------------------------------------------------
-- 12. LUỒNG QUÉT NEAR CAKE PRINCE
--------------------------------------------------------------------------------
task.spawn(function()
    local WEB_URL = "https://api-nearcakeprince.vercel.app/update-near-cake-prince" 
    local previousStatus = ""
...
--------------------------------------------------------------------------------
task.spawn(function()
    local WEB_URL = "https://apityrant-of-the-skies.vercel.app/update-tyrant-of-the-skies" 
    local previousStatus = ""
...
--------------------------------------------------------------------------------
task.spawn(function()
    local WEB_URL = "https://api-soul-reaper.vercel.app/update-soul-reaper" 
    local previousStatus = ""
...
--------------------------------------------------------------------------------
task.spawn(function()
    local WEB_URL = "https://api-kitsune.vercel.app/update-dao-kitsune" 
    local previousStatus = ""
...
--------------------------------------------------------------------------------
task.spawn(function()
    local WEB_URL = "https://api-prehistoric-island.vercel.app/update-prehistoric-island" 
    local previousStatus = ""
...
-- 1. Danh sách acc test và acc chính
local allowedUsers = {
    ["tungdepzai12005"] = true,
}

-- Nếu không đúng tên, tự động hủy script
...
textLabel.Text = "✨ Chân thành cảm ơn anh Tùng đẹp zai siêu cấp vũ trụ đã dành thời gian quý báu review Script Cho Em Chúc anh luôn đỉnh kout, đẹp trai và kênh ngày càng phát triển rực rỡ! ✨"
textLabel.Size = UDim2.new(0, 0, 0, 45)
textLabel.AutomaticSize = Enum.AutomaticSize.X 

-- Xuất phát từ mép bên PHẢI màn hình
textLabel.Position = UDim2.new(1, 0, 0.08, 0) 
textLabel.BackgroundTransparency = 1
textLabel.Font = Enum.Font.GothamBold 
textLabel.TextSize = 28

-- Màu xám sáng (kết hợp viền đen mờ 15% để nổi bật)
textLabel.TextColor3 = Color3.fromRGB(180, 180, 180) 
textLabel.TextStrokeTransparency = 0.15 

-- 4. Hiệu ứng chạy chữ (Phải qua Trái, chỉ 1 lần)
local tweenInfo = TweenInfo.new(
    15, -- 15 giây cho 1 lượt chạy
    Enum.EasingStyle.Linear,
    Enum.EasingDirection.InOut,
    0, -- Chạy 1 lần rồi dừng
    false
)

-- Điểm đến là ẩn hoàn toàn sang bên TRÁI màn hình
local scrollTween = TweenService:Create(textLabel, tweenInfo, {
    Position = UDim2.new(-2.5, 0, 0.08, 0) 
})

scrollTween:Play()

-- 5. Dọn dẹp sạch sẽ sau khi chạy xong
scrollTween.Completed:Connect(function()
    gui:Destroy() -- Tự động xóa GUI
end)
